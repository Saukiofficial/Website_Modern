...
export default function Edit({ osisMember }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        sort_order: osisMember?.sort_order || 0,
        is_leader: Boolean(osisMember?.is_leader),
        is_active: Boolean(osisMember?.is_active),
    });

    const imagePreview = data.image
        ? URL.createObjectURL(data.image)
        : osisMember?.image_url || null;

    const handleSubmit = (event) => {
        event.preventDefault();

        post(`/admin/academics/osis-members/${osisMember.id}`, {
            forceFormData: true,
            preserveScroll: true,
        });
    };
    ...
    return (
        <AdminLayout title="Edit Pengurus OSIS">
            <Head title="Edit Pengurus OSIS" />
            ...
            <div className="mb-6 rounded-[28px] bg-gradient-to-br from-[#061b46] to-[#0b3b85] p-6 text-white shadow-2xl shadow-blue-200 sm:p-8">
                ...
                <p className="mt-4 max-w-2xl text-[14px] font-medium leading-7 text-blue-100">
                    Perbarui data pengurus OSIS yang tampil di halaman
                    akademik.
                </p>
            </div>
